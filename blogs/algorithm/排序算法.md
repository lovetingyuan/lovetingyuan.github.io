...
function quickSort(array, start = 0, end = array.length - 1) {
  if (start >= end) return
  // 取第一个数（或者最后一个数）作为标定值
  // 也可以随机取一个位置，然后跟第一个数或最后一个数交换，然后再开始移动
  const target = array[start]
  let [left, right] = [start + 1, end]
  // 我们的目标就是把>target的值都交换到右边
  while (left < right) {
    if (array[left] > target) {
      ;[array[left], array[right]] = [array[right], array[left]]
      right-- // 交换完右指针可以左移了
    } else {
      left++ // 否则说明左边的数小于target，可以右移左指针
    }
  }
  // 此时左右指针相遇了 left==right，我们需要用最后一个不大于target的值跟标定值交换
  const i = array[left] > target ? left - 1 : left
