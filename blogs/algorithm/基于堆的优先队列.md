...
class Heap {
  constructor(compare, initList = []) {
    this.compare = compare || ((a, b) => a - b) // 默认是小根堆
    this.list = initList
  ...
  heapDown(parentIndex) {
    ...
    if (
      ...
      this.compare(this.list[leftIndex], this.list[targetIndex]) < 0
    ) {
      targetIndex = leftIndex
    }
    if (
      rightIndex < this.list.length &&
      this.compare(this.list[rightIndex], this.list[targetIndex]) < 0
    ) {
